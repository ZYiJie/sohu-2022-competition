...
def get_train_data(input_file):
    ...
    # train = {'content':corpus,'entity':entitys,'content_labels':content_labels, 'entity_labels':entity_labels, 'content_ids':content_ids, 'entity_ids':entity_ids}
    ...
    def forward(self, inputs, labels=None, training=True):
        ...
        if  training:
            ...
            if labels is not None:
                loss1 = self.loss(logits1.permute(0, 2, 1),labels)
                loss2 = self.loss(logits2.permute(0, 2, 1),labels)
                loss3 = self.loss(logits3.permute(0, 2, 1),labels)
                loss4 = self.loss(logits4.permute(0, 2, 1),labels)
                loss5 = self.loss(logits5.permute(0, 2, 1),labels)
